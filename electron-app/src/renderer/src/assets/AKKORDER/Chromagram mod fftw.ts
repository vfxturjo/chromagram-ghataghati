...
export class Chromagram {
  ...
  private centsPerNote: number

  // // TJ ADDED
  // private centsPerNote: number = 10
  // private totalCents: number = 12 * this.centsPerNote // 12 notes * 10 cents per note
   ...
   * @param centsPerNote
   */
  public constructor(frameSize: number, fs: number, centsPerNote: number) {
    ...
    this.complexOut = []
    // TJ ADDED
    this.centsPerNote = centsPerNote
    this.totalCents = 12 * this.centsPerNote
    ...
    for (let i = 0; i < this.totalCents; i++) this.chromagram.push(0.0)

    // setup magnitude spectrum vector
    // setup chromagram vector
    for (let i = 0; i < this.bufferSize / 2 + 1; i++) this.magnitudeSpectrum.push(0)
    ...
    // this.chromaCalculationInterval = 8192
    // this.chromaCalculationInterval = 4096
    this.chromaCalculationInterval = 512
    // this.chromaCalculationInterval = 128
    // this.chromaCalculationInterval = 32
  ...
  private calculateChromagram() {
    ...
    for (let n = 0; n < this.totalCents; n++) {
      let chromaSum = 0.0
      let noteIndex = Math.floor(n / this.centsPerNote)
      let centOffset = n % this.centsPerNote
      let frequencyForCent =
        this.noteFrequencies[noteIndex] * Math.pow(2, centOffset / (12 * this.centsPerNote))

      for (let octave = 1; octave <= this.numOctaves; octave++) {
        let noteSum = 0.0

        for (let harmonic = 1; harmonic <= this.numHarmonics; harmonic++) {
          let centerBin = this.round((frequencyForCent * octave * harmonic) / divisorRatio)
  ...
  private calculateMagnitudeSpectrum() {
    ...
    for (let i = 0; i < this.bufferSize / 2 + 1; i += 1) {
      this.magnitudeSpectrum[i] = Math.sqrt(
        Math.pow(this.complexOut[i * 2], 2) + Math.pow(this.complexOut[i * 2 + 1], 2)
      )
